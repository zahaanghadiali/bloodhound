...
async function handleGlobalCommand(command, conversation) {
  ...
  switch (command) {
    ...
    case 'BACK': {
      const result = flowEngine.back(conversation);
      return [reply(result.ok ? result.prompt : result.message)];
    }
...
async function handle(normalized) {
  ...
  if (command) {
    ...
    const result = flowEngine.advance(conversation, input);
    if (result.error) {
    ...
    const result = stepTypes.choice(input, MENU_STEP);
    if (!result.valid) {
      replies = [reply(`${result.error}\n\n${MENU_STEP.prompt()}`, MENU_STEP.options)];
    } else {
      conversation.consentAcceptedAt = conversation.consentAcceptedAt || new Date();
      const started = flowEngine.start(conversation, result.value);
      replies = [reply(started.prompt)];
    }
  }
